...
static CK_RV
object_handle_to_object(CK_OBJECT_HANDLE handle, struct gpkcs11_st_object **object)
{
...
static int
attributes_match(const struct gpkcs11_st_object *obj, const CK_ATTRIBUTE *attributes, CK_ULONG num_attributes)
{
	CK_ULONG i;
	int j;
...
static void
print_attributes(const CK_ATTRIBUTE *attributes, CK_ULONG num_attributes)
{
	CK_ULONG i;
...
gpkcs11_init_token(const char *version, const char *description, gpkcs11_soft_token_t *token)
{
	...
	ret = sscanf(version, "%c.%c",
		     &gpkcs11_soft_token.desc.libraryVersion.major, &gpkcs11_soft_token.desc.libraryVersion.minor);
	if (ret != 2)
...
C_Finalize(CK_VOID_PTR args)
{
	...
	for (i = 0; i < MAX_NUM_SESSION; i++) {
		if (gpkcs11_soft_token.state[i].session_handle != CK_INVALID_HANDLE) {
			gpkcs11_app_error("application finalized without " "closing session\n");
...
C_GetInfo(CK_INFO_PTR args)
{
	...
	snprintf_fill((char *)args->manufacturerID, sizeof(args->manufacturerID), ' ', MANUFACTURER_ID);
	snprintf_fill((char *)args->libraryDescription, sizeof(args->libraryDescription), ' ', gpkcs11_soft_token.desc.text);
...
C_GetSlotInfo(CK_SLOT_ID slotID, CK_SLOT_INFO_PTR pInfo)
{
	...
	snprintf_fill((char *)pInfo->slotDescription, sizeof(pInfo->slotDescription), ' ', gpkcs11_soft_token.desc.text);
	snprintf_fill((char *)pInfo->manufacturerID, sizeof(pInfo->manufacturerID), ' ', MANUFACTURER_ID);
...
C_GetTokenInfo(CK_SLOT_ID slotID, CK_TOKEN_INFO_PTR pInfo)
{
	...
	snprintf_fill((char *)pInfo->label, sizeof(pInfo->label), ' ', gpkcs11_soft_token.desc.text);
	snprintf_fill((char *)pInfo->manufacturerID, sizeof(pInfo->manufacturerID), ' ', MANUFACTURER_ID);
	snprintf_fill((char *)pInfo->model, sizeof(pInfo->model), ' ', gpkcs11_soft_token.desc.text);
	snprintf_fill((char *)pInfo->serialNumber, sizeof(pInfo->serialNumber), ' ', "3942");
...
C_OpenSession(CK_SLOT_ID slotID,
	      CK_FLAGS flags, CK_VOID_PTR pApplication, CK_NOTIFY Notify, CK_SESSION_HANDLE_PTR phSession)
{
	int i;
	...
	gpkcs11_soft_token.state[i].session_handle = (CK_SESSION_HANDLE) (random() & 0xfffff);
...
C_FindObjectsInit(CK_SESSION_HANDLE hSession, CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount)
{
	...
	if (ulCount) {
		...
		state->find.attributes = calloc(1, ulCount * sizeof(state->find.attributes[0]));
		if (state->find.attributes == NULL)
			return CKR_DEVICE_MEMORY;
		for (i = 0; i < ulCount; i++) {
			state->find.attributes[i].pValue = malloc(pTemplate[i].ulValueLen);
			if (state->find.attributes[i].pValue == NULL) {
				find_object_final(state);
				return CKR_DEVICE_MEMORY;
			}
			memcpy(state->find.attributes[i].pValue, pTemplate[i].pValue, pTemplate[i].ulValueLen);
...
C_FindObjects(CK_SESSION_HANDLE hSession,
	      CK_OBJECT_HANDLE_PTR phObject, CK_ULONG ulMaxObjectCount, CK_ULONG_PTR pulObjectCount)
{
	struct gpkcs11_session_state *state;
	int i;
	...
	*pulObjectCount = 0;
	for (i = state->find.next_object; i < gpkcs11_soft_token.object.num_objs; i++) {
		...
		if (attributes_match(gpkcs11_soft_token.object.objs[i], state->find.attributes, state->find.num_attributes)) {
...
commonInit(CK_ATTRIBUTE *attr_match, int attr_match_len,
	   ...
	   const CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey, struct gpkcs11_st_object **o)
{
	CK_RV ret;
	int i;
	...
	*o = NULL;
	...
	if (!ret) {
		gpkcs11_app_error("called commonInit on key that doesn't " "support required attr");
		return CKR_ARGUMENTS_BAD;
	}
...
C_EncryptInit(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
	...
	ret = commonInit(attr, sizeof(attr) / sizeof(attr[0]),
			 mechs, sizeof(mechs) / sizeof(mechs[0]), pMechanism, hKey, &o);
	if (ret)
		return ret;
	  ...
	  CK_BYTE_PTR pEncryptedData, CK_ULONG ulEncryptedDataLen, CK_BYTE_PTR pData, CK_ULONG_PTR pulDataLen)
{
	...
	len = RSA_private_decrypt(ulEncryptedDataLen, pEncryptedData, buffer, rsa, padding);
	if (len <= 0) {
		ret = CKR_DEVICE_ERROR;
		goto out;
	}
